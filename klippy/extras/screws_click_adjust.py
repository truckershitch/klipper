...
# and these printable bed screw wheels:
...
# Modified by <truckershitch@hambone.e4ward.com>
#
# This file may be distributed under the terms of the GNU GPLv3 license.
import math
from . import probe

class ScrewsClickAdjust:
    def __init__(self, config):
        self.config = config
        self.printer = config.get_printer()
        # self.sta_config = self.printer.lookup_object('screws_tilt_adjust')
        self.screws = []
        self.max_diff = None
        # # Read config
        for i in range(99):
        ...
        if len(self.screws) < 3:
            raise config.error("screws_click_adjust: Must have "
        ...
        # use below if borrowing from [screws_tilt_adjust]

        # # Read config
        # for i in range(99):
        #     prefix = "screw%d" % (i + 1,)
        #     if self.sta_config.get(prefix, None) is None:
        #         break
        #     screw_coord = self.sta_config.getfloatlist(prefix, count=2)
        #     screw_name = "screw at %.3f,%.3f" % screw_coord
        #     screw_name = self.sta_config.get(prefix + "_name", screw_name)
        #     self.screws.append((screw_coord, screw_name))
        # if len(self.screws) < 3:
        #     raise self.sta_config.error("screws_click_adjust: Must have "
        #                        "at least three screws")
        # self.threads = {'CW-M3': 0, 'CCW-M3': 1, 'CW-M4': 2, 'CCW-M4': 3,
        #                 'CW-M5': 4, 'CCW-M5': 5}
        # self.thread = self.sta_config.getchoice('screw_thread', self.threads,
        #                                default='CW-M3')
        # # Initialize ProbePointsHelper
        # points = [coord for coord, name in self.screws]
        # self.probe_helper = probe.ProbePointsHelper(self.sta_config,
        #                                             self.probe_finalize,
        #                                             default_points=points)
        ...
        self.gcode.register_command("SCREWS_CLICK_CALCULATE",
                                    self.cmd_SCREWS_CLICK_CALCULATE,
                                    desc=self.cmd_SCREWS_CLICK_CALCULATE_help)
    cmd_SCREWS_CLICK_CALCULATE_help = "Tool to help adjust bed leveling " \
                                     "screws by calculating the number " \
                                     "of turns and clicks to level it."

    def cmd_SCREWS_CLICK_CALCULATE(self, gcmd):
    ...
    def probe_finalize(self, offsets, positions):
        ...
        self.gcode.respond_info("CW=clockwise, CCW=counter-clockwise")
        for i, screw in enumerate(self.screws):
            ...
            if i == i_base:
                ...
                partial_turn = adjust - full_turns
                clicks = round(partial_turn * 70, 0)
                # Show the results
                # self.gcode.respond_info(
                #     "%s : x=%.1f, y=%.1f, z=%.5f : adjust %s %02d:%02d" %
                #     (name, coord[0], coord[1], z, sign, full_turns, minutes))
                self.gcode.respond_info(
                    "%s : x=%.1f, y=%.1f, z=%.5f : adjust %s %d full turns %d clicks (%d total clicks)" %
                    (name, coord[0], coord[1], z, sign, full_turns, clicks, full_turns * 70 + clicks)
                )
...
def load_config(config):
    return ScrewsClickAdjust(config)
